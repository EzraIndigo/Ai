...
class Example {
	public static void main(String[] args) {
		...
		double[] best = generate_member(solSize);

		for (int j = 0; j < 1000; j++) {


				double[] member = generate_member(solSize);
				System.out.println("Member index " + j + " Fit: " + Assess.getTest1(member));
				if (Assess.getTest1(best) >= Assess.getTest1(member)) {
					best = member;
				}

		}
		double[] secondBest = new double[solSize];

		System.out.println(Assess.getTest1(best));
...
//			System.out.println("The utility is: " + tmp[1]);
	...
	static double[] generate_member(int solSize) {
		double[] sol1 = new double[solSize];
		for (int j = 0; j < solSize; j++) {
